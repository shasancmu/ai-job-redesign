...
export default function AnalyticalEditor({ me, initial, initialStatus }: { me: string; initial: any; initialStatus?: string }) {
  ...
  async function copilot() {
    ...
    try {
      const res = await fetch("/api/mechanics/analytical-copilot", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ intent, currentSpec: spec }) });
      const d = await res.json().catch(() => ({}));
      if (!res.ok || !d.spec) setErrors([d.error || "The copilot couldn't produce an instrument."]);
