...
export default function Lesson4Scale({ session }: { me: string; session: any; initialWorkspace: any }) {
  return (
    <LessonShell session={session} title="Scale, self-play, and the limits" topic="the bitter lesson, scaling laws (and whether they hold), self-play and synthetic data, and what AI can and cannot do: search, structure, think, translate">
      ...
      <p>Put it together and a clear picture emerges. Today&apos;s AI is superb at four things: <strong>search</strong> (finding the relevant pattern in an ocean of text), <strong>structure</strong> (organizing messy input into a clean form), <strong>think</strong> (recombining patterns into a plausible next step), and <strong>translate</strong> (moving an idea between forms, whether code, prose, or a summary).</p>
