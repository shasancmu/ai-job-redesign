  ...
  const renderCard = (m: (typeof MODULES)[number]) => {
    ...
    return (
            <div key={m.slug} className="card group relative flex flex-col p-6 transition hover:shadow-lift">
              ...
              {/* Read the full description on click (opens the detail modal), never on
                  hover — a card that grows under the cursor makes "Start" hard to hit. */}
              <button type="button" onClick={() => setDetail(m.slug)} className="mt-1 self-start text-xs font-medium text-ai hover:underline">More →</button>
