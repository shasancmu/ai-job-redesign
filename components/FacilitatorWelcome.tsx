...
export default function FacilitatorWelcome({ orgs }: { orgs: FacOrg[] }) {
  ...
  return (
    <div className="mb-6 rounded-2xl border border-line bg-white p-4 shadow-soft" style={{ borderLeft: "4px solid var(--brand, #4A6A4E)" }}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <a href="/facilitator" onClick={dismiss} className="btn-primary flex-1 text-center text-sm sm:flex-none">Open the hub →</a>
          <button onClick={dismiss} aria-label="Dismiss" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-400 transition hover:bg-mist hover:text-ink">✕</button>
        </div>
      </div>
    </div>
  );
