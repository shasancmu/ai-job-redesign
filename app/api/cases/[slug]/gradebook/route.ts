...
export async function GET(request: Request, { params }: { params: { slug: string } }) {
  ...
  if ((await caseAuthorId(params.slug)) !== user.id) return new Response("Not yours.", { status: 403 });

  const admin = createAdminClient();
  const { data: mod } = await admin.from("custom_modules").select("spec").eq("slug", params.slug).eq("super_type", LIVING_CASE_TYPE).maybeSingle();
  ...
  // Resolve to classes the instructor owns.
  ...
  const [{ data: members }, { data: profs }, { data: events }] = await Promise.all([
    ...
    admin.from("case_events").select("user_id, kind, created_at").eq("case_slug", params.slug).not("user_id", "is", null).limit(20000),
  ]);
  const nameById = new Map(((profs || []) as any[]).map((p) => [p.id, p.display_name]));

  // Per-student engagement on this case.
  ...
  return new Response(lines.join("\n"), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${params.slug.slice(0, 40)}-gradebook.csv"`,
    },
  });
}
