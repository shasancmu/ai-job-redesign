...
export async function POST(request: Request) {
  try {
    ...
    const calibration = Number.isFinite(body.calibration) ? Math.max(1, Math.min(5, body.calibration)) : undefined;
    const ifPart = String(body.ifPart || "").trim().slice(0, 200);
    const thenPart = String(body.thenPart || "").trim().slice(0, 200);
    const date = String(body.date || "").slice(0, 10);
    const commitment = thenPart
      ? { ifPart, thenPart, date: date || undefined, text: ifPart ? `If ${ifPart}, then I will ${thenPart}` : thenPart }
      : undefined;

    const reflection = { calibration, commitment, at: new Date().toISOString() };
