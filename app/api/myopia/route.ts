import { createClient } from "@/lib/supabase/server";
import { setFlow } from "@/lib/aiflow";
import { AI_ENABLED, myopiaInterviewReply, myopiaReportAI } from "@/lib/ai";
import { streamingResponse } from "@/lib/stream";
import { createAdminClient } from "@/lib/supabase/admin";
import { experimentNudge } from "@/lib/experiments";
import type { MyopiaDomain } from "@/lib/myopia";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 60;

// Overcoming Myopia back end (business + career share it). Auth-gated. Modes:
//  chat   — the diagnostic interview
//  report — the blind-spot diagnosis + exploration plan
export async function POST(request: Request) {
  if (!AI_ENABLED) return Response.json({ error: "AI is not configured." }, { status: 503 });

  const supabase = createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) return Response.json({ error: "Sign in required." }, { status: 401 });

  let body: any;
  try {
    body = await request.json();
  } catch {
    return Response.json({ error: "bad request" }, { status: 400 });
  }
  const mode = String(body.mode || "");
  setFlow("myopia:" + (mode || "chat"));
  const domain: MyopiaDomain = body.domain === "career" ? "career" : "business";
  const subject = String(body.subject || "").slice(0, 400);
  const flow = domain === "career" ? "myopia-career" : "myopia-business";

  try {
    if (mode === "chat") {
      let nudge = "";
      try { nudge = await experimentNudge(createAdminClient(), String(body.sessionId || ""), flow, "interview"); } catch {}
      return streamingResponse((emit) => myopiaInterviewReply(domain, body.messages || [], { subject }, nudge, emit));
    }
    if (mode === "report") {
      let nudge = "";
      try { nudge = await experimentNudge(createAdminClient(), String(body.sessionId || ""), flow, "report"); } catch {}
      const report = await myopiaReportAI({ domain, subject, interview: body.interview || [], nudge });
      if (!report) return Response.json({ error: "Couldn't build the diagnosis. Try again." }, { status: 502 });
      return Response.json({ report });
    }
    return Response.json({ error: "unknown mode" }, { status: 400 });
  } catch (e: any) {
    return Response.json({ error: e?.message || "AI request failed." }, { status: 500 });
  }
}
