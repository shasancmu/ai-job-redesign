    ...
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {report && <ShareReport code={code} title={`${domain === "career" ? "Career" : "Business"} blind spots`} text="Here are the blind spots I found, from Superadditive:" />}
          <Link href={`/room/${code}`} className="btn-ghost text-sm">← Back</Link>
        </div>
      </header>

      <div className="mb-6">
        <div className="text-sm font-semibold uppercase tracking-wide text-slate-400">{domain === "career" ? "Your career's blind spots" : "Your business's blind spots"}</div>
        <h1 className="mt-1 text-3xl text-ink">Overcoming myopia</h1>
      </div>
