...
export const metadata = { title: "Guided interviews" };
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-ink">Guided interviews</h1>
          <p className="mt-1 text-sm text-slate-500">Author-built interview + report modules. No code.</p>
        </div>
        <Link href="/studio/interview/new" className="btn-primary text-sm">+ New module</Link>
      </div>
      ...
      <div className="mt-6 space-y-2">
        {mine.length === 0 && (
          <div className="rounded-xl border border-dashed border-line px-4 py-8 text-center text-sm text-slate-400">
            No modules yet. <Link href="/studio/interview/new" className="font-semibold text-ai hover:underline">Build your first →</Link>
          </div>
        )}
        {mine.map((m) => (
          <div key={m.slug} className="flex items-center justify-between gap-3 rounded-xl border border-line bg-white p-4">
            ...
            <div className="flex flex-none items-center gap-3">
              ...
              <Link href={`/studio/interview/${m.slug}`} className="text-xs text-slate2 hover:text-ink">Edit</Link>
            </div>
          </div>
        ))}
      </div>
