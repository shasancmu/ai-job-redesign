...
6. Reports on equity, bias, and trust & \stSub & 55 \\
7. Pilot integrations with TTOs / research administrators & \stPart & 25 \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Overall (simple average)}} & \textbf{$\approx$78} \\
\bottomrule
\end{tabular}
\end{center}

\noindent \textbf{Five deliverables are complete and a sixth (equity/bias) is now
underway with a first audit run (Section~\ref{sec:bias}).} Weighted by technical
difficulty completion is higher still: the three new predictive models (the
proposal's central scientific contribution), the agentic interface, ExplainAI, the
interactive maps, and the open code/data are all delivered. The clearest remaining
work is the \emph{in-field TTO pilots} (running the tools on real disclosure data
with administrators in the loop) and the trust/calibration half of the equity report.
...
\subsection{6. Reports on equity, bias, and trust \hfill \stSub}
...
\end{itemize}
\textbf{Status:} the equity/bias audit has now been \emph{run} --- see
Section~\ref{sec:bias} for the method, results, and finding. The remaining piece is
the formal trust/calibration analysis (reliability curves per model).
...
\section{Equity \& bias audit --- first results}\label{sec:bias}
...
author-demographic cut is not yet possible from this data. The complementary
\emph{trust} analysis (per-model calibration/reliability curves) is the remaining
piece of Deliverable~6.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=1pt]
  ...
  \item \textbf{Trust/calibration curves} for the three models --- the remaining half of Del.\ 6.
