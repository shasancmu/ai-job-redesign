...
6. Reports on equity, bias, and trust & \stSub & 80 \\
7. Pilot integrations with TTOs / research administrators & \stPart & 25 \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Overall (simple average)}} & \textbf{$\approx$81} \\
\bottomrule
\end{tabular}
\end{center}

\noindent \textbf{Six of seven deliverables are complete or substantially so.} The
equity/bias audit and the trust/calibration analysis are both run
(Section~\ref{sec:bias}). Weighted by technical difficulty completion is higher
still: the three new predictive models (the proposal's central scientific
contribution), the agentic interface, ExplainAI, the interactive maps, and the open
code/data are all delivered. The single clearest remaining item is the
...
\end{itemize}
\textbf{Status:} \emph{run} --- see Section~\ref{sec:bias}. The equity/bias audit
found no under-crediting of small-institution science, and the trust analysis shows
all three models are well-calibrated (ECE $\leq 5\%$). What remains is an
author-demographic cut of the equity analysis and writing it up as a standalone
paper.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=1pt]
  ...
  \item \textbf{Author-demographic cut} of the equity analysis + a standalone write-up (Del.\ 6).
