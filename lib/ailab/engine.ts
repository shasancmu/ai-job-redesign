...
export async function runVibe(challenge: Challenge, input: { spec: Record<string, string>; prompt: string }): Promise<RunResult> {
  ...
  const system = `You are a build tool like Lovable/v0, but you MAKE YOUR ASSUMPTIONS VISIBLE for teaching. Given a spec and a prompt, produce ONE small self-contained HTML page (inline CSS only, no external scripts, no network) that renders the requested screen with realistic placeholder data. Keep it to what's asked; don't invent extra features. Also list the assumptions you had to make that the learner did NOT specify.\n\nReturn STRICT JSON only:\n{"html":"<!doctype html>...","assumptions":["short specific assumption",...],"notes":"one line on what you built"}\nThe html must be a complete standalone document under 12000 characters.`;
