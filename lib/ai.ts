...
const PX_SPEC_SHAPE = `Return STRICT JSON with EXACTLY these keys:
{
  ...
  "evidence": { "headline": "the key result", "chart": { "kind": "slope|bars|stacked|hbars|line|area|scatter|coef", "title": "chart title", "xLabel": "x axis label if useful", "yLabel": "y axis label", "annotation": "one callout like +42% or -1.28 SD", "series": [ { "label": "series name", "tone": "up|down|neutral", "trend": false, "points": [ { "x": "label or number", "y": 0, "lo": 0, "hi": 0 } ] } ] }, "takeaway": "40-70 words on what the evidence shows" },
...
- The chart should RECONSTRUCT ONE REAL FIGURE OR RESULT from the paper — ideally redraw an actual figure the paper shows (e.g. its time trend), using the paper's own real data points. It is OPTIONAL: a clear, faithful chart of one result beats a clever one, and NO chart is better than a confusing or fabricated one, so OMIT the chart key unless you can build a clean, honest one from real numbers.
- ONE METRIC PER CHART. Never combine two different quantities or units in a single chart (do NOT put a "+42% volume" change and a "-1.28 SD quality" change together — pick the single most important one, or use two series only if they share the same unit and axis). The annotation is one short callout for that one metric.
- Choose the type that matches the result: 'line'/'area' for a value over ordered time (e.g. monthly submissions rising after a date — usually the best choice for a trend); 'slope' for a single before/after change on ONE metric (two points: "Before"/"After"); 'bars' for a value across a few categories (grouped only if 2-3 series share the axis); 'stacked' for shares that sum to a whole; 'hbars' for a ranking with labels; 'scatter' for a relationship between two numeric variables (numeric value in x, set trend:true to draw the fitted line). Use 'coef' ONLY for a set of at least 2 real regression coefficients / effect sizes, each with a genuine confidence interval lo..hi and a common axis; never use 'coef' for a single summary statistic. Only include lo/hi for 'coef'. Keep it to 2-12 points per series, at most 3 series.
