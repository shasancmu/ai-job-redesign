...
- The chart is OPTIONAL but strongly preferred when the paper has a clear quantitative result: pick the ONE result that best captures the finding, choose the chart type that shows it most honestly, and encode it as a few REAL data points. Chart menu: 'slope' for a before/after two-point change; 'bars' for a few categories (grouped if 2-3 series); 'stacked' for composition/shares that sum to a whole; 'hbars' for a ranking or many labeled categories; 'line' for a trend over ordered time; 'area' for a trend you want to emphasize as a rising/falling volume; 'scatter' for a relationship between two numeric variables (put the numeric value in x, and set trend:true on a series to draw its fitted line); 'coef' for a coefficient/effect-size plot where each point has an estimate y and a confidence interval lo..hi (a zero reference line is drawn). Keep it to 2-12 points per series and at most 3 series. Only include lo/hi for 'coef'. If you have no trustworthy numbers, OMIT the chart key.
